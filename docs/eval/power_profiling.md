...
measurements to be taken during `lemonade-eval` execution. It is only available
...
To capture power data during the `lemonade-eval` execution sequence with the HWiNFO tool:
...
3) Run `lemonade-eval` from a CMD or PowerShell prompt with Administrator privileges.

You may then use the `--power-hwinfo` flag.  For example:

OGA iGPU:
```bash
lemonade-eval --power-hwinfo -i microsoft/Phi-3-mini-4k-instruct oga-load --device igpu --dtype int4 llm-prompt -p "Hello"
```

This generates a PNG file that is stored in the current folder.  This file
contains a figure plotting the power usage over the Lemonade tool sequence.  Learn more by running `lemonade-eval -h`.

## Power Profiling with AGT

Please note that this tool only works on AMD platforms and is only available to AMD employees.

To capture power data during the `lemonade-eval` execution sequence with the AGT tool:
...
OGA iGPU:
```bash
lemonade-eval --power-agt -i microsoft/Phi-3-mini-4k-instruct oga-load --device igpu --dtype int4 llm-prompt -p "Hello"
```
...
`lemonade-eval` to automatically launch HWiNFO.
